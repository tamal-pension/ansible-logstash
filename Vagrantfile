...
MAIN_SH_ARGS = <<MARKER
-e "playbook_name=ansible-logstash discord_message_owner_name=#{Etc.getpwuid(Process.uid).name} environment_id=pension-stg.local" --tags "installation,configuration,debug"
MARKER
Vagrant.configure("2") do |config|
  ...
  config.vm.provider :aws do |aws, override|
    ...
    aws.security_groups = ["sg-077f8d7d58d420467","sg-09475a892a269d217"]
    # public-ssh, logstash
    ...
    aws.tags = {
      ...
      version: "latest",
      app: "logstash",
      private_dns: "logstash-test-#{Etc.getpwuid(Process.uid).name}",
    }
  end
end